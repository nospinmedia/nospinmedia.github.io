...
The KB's per-article category is a single `post_tag` value (e.g. "Government", "Politics"), separate from the free-form `tags` array used here. "Voting & Elections" is a curated cross-cutting topic layered on top of `post_tag` in `knowledge.html` itself: it's driven by the `tags` array containing the literal string `"voting & elections"` (added to 12 articles directly in the KB's live data, confirmed against a genuinely qualifying set before applying) rather than reassigning any article's actual `post_tag`. The "Explore Voting & Election Explainers" link uses `knowledge.html?topic=voting-and-elections`, a URL-addressable topic-filter parameter added to `knowledge.html` for this (falls back to "all" silently for an unrecognized slug; existing `?id=` permalinks, search, and other topic filters are unaffected).
